...
survfitcurePK <- function(x, t, d, xinu, dataset = NULL, x0, h, local = TRUE) {
  dfr <-
    if (missing(dataset)) {
      na.omit(data.frame(x, t, d, xinu))
    } else {
      na.omit(dataset[, c(deparse(substitute(x)), deparse(substitute(t)), deparse(substitute(d)), deparse(substitute(xinu)))])
    }
...
survfitcurePK_un <- function(dataset = dataset) {
  ...
  for (i in 2:n) {
    ...
    if(d[i]==1) {
      S[i] <- S[i-1] * (1 - 1/(n - i + 1 + cum.nu[i-1]))
    }
  }
  p <- 1 - min(S)
  return(list(S, p, t))
...
survfitcurePKinhbootmise <- function(dfr, x0, g) {
  ...
  Sg <- out$S
  pg <-  out$p
  dfrboot <-  data.frame(matrix(0, nrow = n_row, ncol = n_col, dimnames = list(NULL, c("x", "t", "d", "xinu"))))
  if(pg == 0) {
    S0g <- pg*Sg
    ...
    tmax <- data.table::first(ord.dfr$t[S0g == data.table::first(DescTools::Closest(S0g, 1 - 0.9, na.rm = TRUE))])
  }
...
survfitcurePKboot <- function(dfr, x0, ncpus = 2L) {
  ...
  out <- foreach::foreach(b = 1 : B, .packages = c("npcure", "DescTools","data.table", "doParallel"), .export = ls(globalenv()), .options.snow = opts, .verbose = TRUE)%dopar%{
...
latencyinbootfun <- function(dfr = dfr,   x0 = x0, hS = hS, hp = hp) {
  ...
  tgrid <- seq(0, tmax, length.out = 100L)
  weights.matrix <- outer(dfr$x, dfr$x,  weights.function, K = kernel, h = hS)
  dfrboot[ , 1] <-  dfr$x
  for (i in 1:n_row) {
    dfrboot[i, 2:4] <- dfr[sample(nrow(dfr), size = 1, prob = weights.matrix[i, ]), 2:4]
  }
  ...
  for (ih in 1:lhgrid) {
    for (ij in 1:lhgrid) {
      ...
      S0h.grid <- approxfun(x = c(0, ord.dfrboot[, 2]), y = c(1, S0h), method = "constant", f = 0, rule = 2, ties = "ordered")
      S0g.grid <- approxfun(x = c(0, ord.dfr$t), y = c(1, S0g), method = "constant", f = 0, rule = 2, ties = "ordered")
      diff <- (S0h.grid(tgrid) - S0g.grid(tgrid))^2
      diff.grid <- approxfun(x = tgrid, y = diff, method = "constant", f = 0, rule = 2, ties = "ordered")
      se <- function(t) {
        diff.grid(t)
      }
      bootmise[ij, ih] <-  as.numeric(integrate(se, 0 + .Machine$double.eps^0.3, tmax, rel.tol = 0.1, subdivisions = 2000)[1])
    } # for each ij
  } # for each ih
...
latencybootfun <- function(dfr, x0, ncpus = 2L) {
  ...
  mise <- miseres / B
  min_mise <- min(mise)
  ind <- colMeans(which(min_mise == mise, arr.ind = TRUE))
  hboot <- hgrid[ind]
  close(pb)
  parallel::stopCluster(cl)
  return(hboot = hboot)
